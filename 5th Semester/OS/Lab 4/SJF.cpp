...
bool CompareProcessBT(Process &p1, Process &p2) {
    return (p1.getBT() < p2.getBT());
}
...
void SortBT(std::vector<Process> &Processes, const int &toe, int s) {
    int i = s;

    for (i = s; i < Processes.size(); ++i) {
        if (Processes[i].getAT() >  toe) {
            break;
        }
    }

    sort(Processes.begin()+s, Processes.begin()+i, CompareProcessBT);
}

void ScheduleProcessSJF(std::vector<Process> &Processes) {
    ...
    for (int i = 0; i < Processes.size(); ++i) {
        ...
        SortBT(Processes, timeOfExecution, i+1);
    }
}
